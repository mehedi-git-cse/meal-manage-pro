...
<div class="max-w-xl mx-auto animate-fade-in space-y-5">
    ...
    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route('deposits.update', encryptId($deposit->id)) }}" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select name="user_id" class="form-select @error('user_id') border-red-400 @enderror" required>
                        ...
                        @foreach($users as $u)
                            <option value="{{ encryptId($u->id) }}"
                                {{ (old('user_id', encryptId($deposit->user_id)) == encryptId($u->id)) ? 'selected' : '' }}>
                                {{ $u->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="amount" value="{{ old('amount', $deposit->amount) }}"
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="deposit_date" value="{{ old('deposit_date', $deposit->deposit_date->format('Y-m-d')) }}"
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="note" value="{{ old('note', $deposit->note) }}"
                           class="form-input @error('note') border-red-400 @enderror">
                    @error('note')<p class="form-error">{{ $message }}</p>@enderror
                </div>

                <div class="flex gap-3">
                    <button type="submit" class="btn-primary flex-1">Update Deposit</button>
